...
/**
 * Compact best-guide commerce JSON. Region comes only from the path.
 * Does not read cookies or call getBestGuidePageData.
 */
export async function GET(_request: Request, context: RouteProps) {
  ...
  const data = getBestGuideCatalogPrices(slug, region);
  if (!data) {
