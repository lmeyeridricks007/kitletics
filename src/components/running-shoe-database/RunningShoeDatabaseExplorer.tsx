"use client";

import type { ReactNode } from "react";
import {
  useCallback,
  useEffect,
  useId,
  useMemo,
  useRef,
  useState,
  useTransition,
} from "react";
import { usePathname, useRouter, useSearchParams } from "next/navigation";
import { SlidersHorizontal, X } from "lucide-react";
import {
  DATABASE_SORT_OPTIONS,
  buildActiveDatabaseChips,
  humanizeToken,
  queryRunningShoeDatabase,
  removeDatabaseFilterValue,
  suggestRestrictiveFilters,
} from "@/lib/running-shoe-database/query";
import {
  DROP_BUCKETS,
  PRICE_BUCKETS,
  STACK_BUCKETS,
  WEIGHT_BUCKETS,
  countActiveDatabaseFilters,
  databaseHref,
  parseDatabaseSearchParams,
  DEFAULT_DATABASE_FILTERS,
} from "@/lib/running-shoe-database/params";
import type {
  RunningShoeDatabaseFilters,
  RunningShoeDatabasePageData,
  RunningShoeDatabaseSort,
} from "@/lib/running-shoe-database/types";
import {
  DatabaseCompareSticky,
  RunningShoeDatabaseCard,
} from "@/components/running-shoe-database/RunningShoeDatabaseCard";
import { useModalFocus } from "@/lib/a11y/use-modal-focus";
import { trackDatabaseFilterChange } from "@/lib/running-shoe-database/analytics";
import { cn } from "@/lib/utils";

function toggleValue(list: string[], value: string): string[] {
  return list.includes(value)
    ? list.filter((v) => v !== value)
    : [...list, value];
}

function ChipToggle({
  active,
  label,
  count,
  onClick,
}: {
  active: boolean;
  label: string;
  count?: number;
  onClick: () => void;
}) {
  return (
    <button
      type="button"
      onClick={onClick}
      aria-pressed={active}
      className={cn(
        "inline-flex items-center gap-1.5 border px-2.5 py-1.5 text-[12px] font-medium transition-colors",
        active
          ? "border-foreground bg-foreground text-white"
          : "border-border bg-white text-foreground hover:border-foreground/40",
      )}
    >
      {label}
      {typeof count === "number" && (
        <span
          className={cn(
            "tabular-nums",
            active ? "text-white/70" : "text-muted",
          )}
        >
          {count}
        </span>
      )}
    </button>
  );
}

function FilterSection({
  title,
  children,
}: {
  title: string;
  children: ReactNode;
}) {
  return (
    <div>
      <p className="text-[11px] font-bold tracking-[0.1em] text-muted uppercase">
        {title}
      </p>
      <div className="mt-2 flex max-h-44 flex-wrap gap-1.5 overflow-y-auto">
        {children}
      </div>
    </div>
  );
}

function FilterPanel({
  data,
  filters,
  onChange,
}: {
  data: RunningShoeDatabasePageData;
  filters: RunningShoeDatabaseFilters;
  onChange: (next: RunningShoeDatabaseFilters) => void;
}) {
  const setList = (
    key: keyof RunningShoeDatabaseFilters,
    value: string,
  ) => {
    const current = filters[key];
    if (!Array.isArray(current)) return;
    onChange({
      ...filters,
      [key]: toggleValue(current as string[], value),
    });
  };

  return (
    <div className="space-y-6">
      <label className="block">
        <span className="text-[11px] font-bold tracking-[0.1em] text-muted uppercase">
          Search
        </span>
        <input
          type="search"
          value={filters.q ?? ""}
          onChange={(e) =>
            onChange({
              ...filters,
              q: e.target.value.trim() ? e.target.value : undefined,
            })
          }
          placeholder="Brand or model"
          className="mt-1.5 w-full border border-border bg-white px-3 py-2 text-[14px] outline-none focus:border-foreground"
        />
      </label>

      <FilterSection title="Gender / fit">
        {data.facets.gender.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.gender.includes(
              opt.value as "men" | "women" | "unisex",
            )}
            label={
              opt.value === "men"
                ? "Men"
                : opt.value === "women"
                  ? "Women"
                  : "Unisex"
            }
            count={opt.count}
            onClick={() => setList("gender", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Use">
        {data.facets.useCases.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.useCase.includes(opt.value)}
            label={opt.label}
            count={opt.count}
            onClick={() => setList("useCase", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Shoe type">
        {data.facets.types.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.type.includes(opt.value)}
            label={opt.label}
            count={opt.count}
            onClick={() => setList("type", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Cushion">
        {data.facets.cushion.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.cushion.includes(opt.value)}
            label={humanizeToken(opt.value)}
            count={opt.count}
            onClick={() => setList("cushion", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Stability">
        {data.facets.stability.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.stability.includes(opt.value)}
            label={humanizeToken(opt.value)}
            count={opt.count}
            onClick={() => setList("stability", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Weight">
        {WEIGHT_BUCKETS.map((b) => (
          <ChipToggle
            key={b.id}
            active={filters.weightBuckets.includes(b.id)}
            label={b.label}
            onClick={() => setList("weightBuckets", b.id)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Drop">
        {DROP_BUCKETS.map((b) => (
          <ChipToggle
            key={b.id}
            active={filters.dropBuckets.includes(b.id)}
            label={b.label}
            onClick={() => setList("dropBuckets", b.id)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Heel stack">
        {STACK_BUCKETS.map((b) => (
          <ChipToggle
            key={b.id}
            active={filters.stackBuckets.includes(b.id)}
            label={b.label}
            onClick={() => setList("stackBuckets", b.id)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Price">
        {PRICE_BUCKETS.map((b) => (
          <ChipToggle
            key={b.id}
            active={filters.priceBuckets.includes(b.id)}
            label={b.label}
            onClick={() => setList("priceBuckets", b.id)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Race distance">
        {data.facets.distance.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.distance.includes(opt.value)}
            label={humanizeToken(opt.value)}
            count={opt.count}
            onClick={() => setList("distance", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Surface">
        {data.facets.surface.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.surface.includes(opt.value)}
            label={humanizeToken(opt.value)}
            count={opt.count}
            onClick={() => setList("surface", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Brand">
        {data.facets.brands.map((opt) => (
          <ChipToggle
            key={opt.value}
            active={filters.brand.includes(opt.value)}
            label={opt.label}
            count={opt.count}
            onClick={() => setList("brand", opt.value)}
          />
        ))}
      </FilterSection>

      <FilterSection title="Plate">
        {(
          [
            ["any", "Any"],
            ["carbon", "Carbon"],
            ["plated", "Any plate"],
            ["none", "No plate"],
          ] as const
        ).map(([value, label]) => (
          <ChipToggle
            key={value}
            active={filters.plate === value}
            label={label}
            onClick={() => onChange({ ...filters, plate: value })}
          />
        ))}
      </FilterSection>
    </div>
  );
}

export function RunningShoeDatabaseExplorer({
  data,
}: {
  data: RunningShoeDatabasePageData;
}) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [filters, setFilters] = useState<RunningShoeDatabaseFilters>(() =>
    parseDatabaseSearchParams(
      Object.fromEntries(searchParams.entries()),
    ),
  );
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [, startTransition] = useTransition();
  const titleId = useId();
  const drawerRef = useRef<HTMLDivElement>(null);
  const closeDrawer = useCallback(() => setDrawerOpen(false), []);
  useModalFocus(drawerOpen, drawerRef, closeDrawer);

  // Sync from browser back/forward
  useEffect(() => {
    const next = parseDatabaseSearchParams(
      Object.fromEntries(searchParams.entries()),
    );
    setFilters(next);
  }, [searchParams]);

  const pushFilters = useCallback(
    (next: RunningShoeDatabaseFilters) => {
      const prev = filters;
      const nextResults = queryRunningShoeDatabase(data.records, next);
      trackDatabaseFilterChange(prev, next, nextResults.length);
      setFilters(next);
      startTransition(() => {
        router.push(databaseHref(next, pathname || data.path), {
          scroll: false,
        });
      });
    },
    [router, pathname, data.path, data.records, filters],
  );

  const results = useMemo(
    () => queryRunningShoeDatabase(data.records, filters),
    [data.records, filters],
  );

  const labelMaps = useMemo(
    () => ({
      brand: Object.fromEntries(
        data.facets.brands.map((b) => [b.value, b.label]),
      ),
      type: Object.fromEntries(
        data.facets.types.map((b) => [b.value, b.label]),
      ),
      useCase: Object.fromEntries(
        data.facets.useCases.map((b) => [b.value, b.label]),
      ),
    }),
    [data.facets],
  );

  const chips = useMemo(
    () => buildActiveDatabaseChips(filters, labelMaps),
    [filters, labelMaps],
  );
  const activeCount = countActiveDatabaseFilters(filters);
  const restrictive = suggestRestrictiveFilters(filters, chips);

  return (
    <>
      <div className="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-10">
        <aside className="hidden lg:sticky lg:top-[calc(var(--site-chrome-height)+1rem)] lg:block lg:self-start">
          <div className="flex items-center justify-between gap-3">
            <h2 className="font-display text-lg font-bold tracking-tight">
              Filters
            </h2>
            {activeCount > 0 && (
              <button
                type="button"
                className="text-[12px] font-semibold text-link hover:text-link-hover"
                onClick={() => pushFilters(DEFAULT_DATABASE_FILTERS)}
              >
                Clear all
              </button>
            )}
          </div>
          <div className="mt-4">
            <FilterPanel
              data={data}
              filters={filters}
              onChange={pushFilters}
            />
          </div>
        </aside>

        <div className="min-w-0">
          <div className="flex flex-wrap items-end justify-between gap-3 border-b border-border pb-4">
            <div>
              <p className="text-[11px] font-bold tracking-[0.14em] text-muted uppercase">
                Results
              </p>
              <p className="mt-1 font-display text-2xl font-bold tracking-tight tabular-nums">
                {results.length}
                <span className="ml-2 text-base font-medium text-muted">
                  of {data.total} shoes
                </span>
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button
                type="button"
                className="inline-flex h-10 items-center gap-2 border border-border bg-white px-3 text-[12px] font-semibold lg:hidden"
                onClick={() => setDrawerOpen(true)}
                aria-haspopup="dialog"
              >
                <SlidersHorizontal className="size-4" aria-hidden />
                Filters
                {activeCount > 0 ? ` (${activeCount})` : ""}
              </button>
              <label className="flex items-center gap-2 text-[13px]">
                <span className="text-muted">Sort</span>
                <select
                  value={filters.sort}
                  onChange={(e) =>
                    pushFilters({
                      ...filters,
                      sort: e.target.value as RunningShoeDatabaseSort,
                    })
                  }
                  className="h-10 border border-border bg-white px-2.5 text-[13px] font-medium outline-none focus:border-foreground"
                >
                  {DATABASE_SORT_OPTIONS.map((o) => (
                    <option key={o.value} value={o.value}>
                      {o.label}
                    </option>
                  ))}
                </select>
              </label>
            </div>
          </div>

          {chips.length > 0 && (
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {chips.map((chip) => (
                <button
                  key={chip.id}
                  type="button"
                  onClick={() =>
                    pushFilters(removeDatabaseFilterValue(filters, chip))
                  }
                  className="inline-flex items-center gap-1.5 border border-foreground/20 bg-[#f5f6f7] px-2 py-1 text-[12px] font-medium"
                >
                  {chip.label}
                  <X className="size-3.5" aria-hidden />
                  <span className="sr-only">Remove {chip.label}</span>
                </button>
              ))}
              <button
                type="button"
                className="text-[12px] font-semibold text-link"
                onClick={() => pushFilters(DEFAULT_DATABASE_FILTERS)}
              >
                Clear all
              </button>
            </div>
          )}

          <div className="mt-4 space-y-3 pb-24 sm:pb-8">
            {results.length === 0 ? (
              <div className="border border-dashed border-border px-6 py-14">
                <p className="font-display text-xl font-bold tracking-tight">
                  No shoes match all these filters.
                </p>
                <p className="mt-2 max-w-lg text-[14px] text-muted">
                  Try clearing the most restrictive filters, or loosen one
                  dimension at a time.
                </p>
                {restrictive.length > 0 && (
                  <div className="mt-4">
                    <p className="text-[11px] font-bold tracking-[0.1em] text-muted uppercase">
                      Likely restrictive
                    </p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {restrictive.map((chip) => (
                        <button
                          key={chip.id}
                          type="button"
                          className="border border-border bg-white px-2.5 py-1.5 text-[12px] font-medium hover:border-foreground/40"
                          onClick={() =>
                            pushFilters(
                              removeDatabaseFilterValue(filters, chip),
                            )
                          }
                        >
                          Remove {chip.label}
                        </button>
                      ))}
                    </div>
                  </div>
                )}
                <button
                  type="button"
                  className="mt-5 text-[13px] font-semibold text-link"
                  onClick={() => pushFilters(DEFAULT_DATABASE_FILTERS)}
                >
                  Clear all filters
                </button>
              </div>
            ) : (
              results.map((record, index) => (
                <RunningShoeDatabaseCard
                  key={record.id}
                  record={record}
                  resultPosition={index + 1}
                />
              ))
            )}
          </div>
        </div>
      </div>

      {drawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="presentation">
          <button
            type="button"
            className="absolute inset-0 bg-black/40"
            aria-label="Close filters"
            onClick={closeDrawer}
          />
          <div
            ref={drawerRef}
            role="dialog"
            aria-modal="true"
            aria-labelledby={titleId}
            tabIndex={-1}
            className="absolute inset-x-0 bottom-0 max-h-[88vh] overflow-y-auto border-t border-border bg-white px-4 pb-8 pt-4 shadow-lg outline-none"
          >
            <div className="mb-4 flex items-center justify-between gap-3">
              <h2 id={titleId} className="font-display text-lg font-bold">
                Filters
              </h2>
              <button
                type="button"
                onClick={closeDrawer}
                className="inline-flex size-9 items-center justify-center border border-border"
                aria-label="Close"
              >
                <X className="size-4" />
              </button>
            </div>
            <FilterPanel
              data={data}
              filters={filters}
              onChange={pushFilters}
            />
            <div className="sticky bottom-0 mt-6 flex gap-2 bg-white pt-3">
              {activeCount > 0 && (
                <button
                  type="button"
                  className="h-11 flex-1 border border-border text-[12px] font-semibold"
                  onClick={() => pushFilters(DEFAULT_DATABASE_FILTERS)}
                >
                  Clear all
                </button>
              )}
              <button
                type="button"
                className="h-11 flex-[2] bg-accent text-[12px] font-bold tracking-[0.06em] text-accent-foreground uppercase"
                onClick={closeDrawer}
              >
                Show {results.length} shoes
              </button>
            </div>
          </div>
        </div>
      )}

      <DatabaseCompareSticky />
    </>
  );
}
