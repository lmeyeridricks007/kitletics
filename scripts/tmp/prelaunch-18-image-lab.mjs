/**
 * Pre-launch 18 — Image transfer lab + budget check.
 *
 * Usage:
 *   BASE_URL=http://127.0.0.1:3010 node scripts/tmp/prelaunch-18-image-lab.mjs
 *
 * Writes:
 *   docs/prelaunch/data/18-image-lab.json
 * Exit 1 if any route exceeds ROUTE_TRANSFER_BUDGETS (when ENFORCE=1).
 */
...
for (const path of ROUTES) {
  ...
  results.push(row);
  console.log(
    `${path} total=${row.totalMB}MB images=${row.imageMB}MB next=${row.nextImageCount} raw=${row.rawImageCount} budget=${row.withinBudget ? "OK" : "OVER"}`,
  );
}
...
writeFileSync(
  join(OUT_DIR, "18-image-lab.json"),
  JSON.stringify(payload, null, 2),
);
console.log("Wrote docs/prelaunch/data/18-image-lab.json");

if (ENFORCE && payload.failures.length) {
  console.error("Budget failures:", payload.failures.join(", "));
  process.exit(1);
}
