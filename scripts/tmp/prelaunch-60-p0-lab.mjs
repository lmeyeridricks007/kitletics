/**
 * Fix 60 — P0 transfer size + axe (includes review).
 *
 * BASE_URL=http://127.0.0.1:3010 node scripts/tmp/prelaunch-60-p0-lab.mjs
 */
...
async function measure(page, path) {
  ...
  try {
    ...
    axe = {
      ...
      seriousCriticalHelp: serious.map((v) => ({
        id: v.id,
        impact: v.impact,
...
async function main() {
  ...
  const report = {
    ...
    gates: {
      ...
      compareWithinBudget:
        (routes.find((r) => r.id === "compare")?.totalBytes ?? Infinity) <=
        BUDGET,
      dailyTrainersWithinBudget:
        (routes.find((r) => r.id === "daily-trainers")?.totalBytes ??
          Infinity) <= BUDGET,
    },
